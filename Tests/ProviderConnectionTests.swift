...
struct ProviderConnectionTests {
    ...
    static func main() throws {
        ...
        store.set(nil, at: 0)
        expect(store.leftSlot == nil && store.rightSlot == .codex, "left slot can be cleared while right remains")
        store.set(.codex, at: 0)
        expect(store.leftSlot == .codex && store.rightSlot == nil, "single provider can move between slots")
        store.set(.codex, at: 0)
        ...
        expect(store.leftSlot == nil && store.rightSlot == .codex,
               "choosing the same provider in the empty slot moves it")
        store.set(.codex, at: 0)
